...
save_as_csv(tweet.Kata, "cleaningdata\\clean_data.csv", prepend_ids = TRUE, na = "", fileEncoding = "UTF-8")
head(tweet.Kata$text)
...
hotel_dataset <-read.csv("cleaningdata\\clean_data.csv",stringsAsFactors = FALSE)
...
ui <- fluidPage(
  ...
  mainPanel(
    tabsetPanel(type = "tabs",
                ...
                tabPanel("Comparation Word", plotOutput("comparation"))
    )
  )
)
